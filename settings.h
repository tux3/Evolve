#ifndef SETTINGS_H
#define SETTINGS_H

extern int N_POLY_POINTS;
extern int N_COLOR_VAR;
extern int N_POS_VAR;
extern int SHAPE_OPT_FREQ;

extern int GUI_REFRESH_RATE;

extern int AUTOFOCUS_RES;
extern int FOCUS_LEFT, FOCUS_RIGHT, FOCUS_TOP, FOCUS_BOTTOM;

extern int N_CORES;

extern int POLYS_MIN, POLYS_MAX;

extern int POLYS_ADD_RATE, POLYS_REMOVE_RATE, POLYS_REORDER_RATE;
extern int POINT_MOVE_MAX_RATE, POINT_MOVE_MED_RATE, POINT_MOVE_MIN_RATE;
extern int POINT_MOVE_MED_RANGE, POINT_MOVE_MIN_RANGE;
extern int POINT_ADD_RATE, POINT_DELETE_RATE;
extern int RED_CHANGE_RATE, GREEN_CHANGE_RATE, BLUE_CHANGE_RATE, ALPHA_CHANGE_RATE;
extern int ALPHA_MIN, ALPHA_MAX;

#define GEN_WITH_RANDOM_COLOR false
#define OPT_INCREASE_ALPHA true

#endif // SETTINGS_H
